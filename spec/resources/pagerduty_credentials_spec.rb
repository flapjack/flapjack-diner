    ...
    it 'submits a PUT request for a set of pagerduty credentials' do
      flapjack.given("a set of pagerduty credentials with id '#{pagerduty_credentials_data[:id]}' exists").
        ...
        with(:method => :put,
             ...
             :body => {:pagerduty_credentials => {:id => pagerduty_credentials_data[:id], :username => 'acegi'}},
             :headers => {'Content-Type' => 'application/vnd.api+json'}).
        will_respond_with(
          :status => 204,
          :body => '' )

      result = Flapjack::Diner.update_pagerduty_credentials(:id => pagerduty_credentials_data[:id], :username => 'acegi')
    ...
    it 'submits a PUT request for several sets of pagerduty credentials' do
      flapjack.given("two sets of pagerduty credentials with ids '#{pagerduty_credentials_data[:id]}' and '#{pagerduty_credentials_2_data[:id]}' exist").
        ...
        with(:method => :put,
             ...
             :body => {:pagerduty_credentials => [{:id => pagerduty_credentials_data[:id], :username => 'acegi'},
             {:id => pagerduty_credentials_2_data[:id], :username => 'kmoqs'}]},
             :headers => {'Content-Type' => 'application/vnd.api+json'}).
        will_respond_with(
          :status => 204,
          :body => '' )

      result = Flapjack::Diner.update_pagerduty_credentials(
        {:id => pagerduty_credentials_data[:id], :username => 'acegi'},
        {:id => pagerduty_credentials_2_data[:id], :username => 'kmoqs'})
      expect(result).to be_a(TrueClass)
    end
